...
getFileDate() {
    ...
    FILEHOSTNAMEPATH=$(echo "${1}" | awk -F${LOCALDIR} '{print $NF}')
    FILEHOSTNAME=$(echo "${FILEHOSTNAMEPATH%%/*}")
    FILEYEARPATH=$(echo "${1}" | awk -F${BACKUPHOSTNAME}/ '{print $NF}')
    FILEYEAR=$(echo "${FILEYEARPATH%%/*}" | cut -d - -f 1)
    FILEMONTH=$(echo "${FILEYEARPATH%%/*}" | cut -d - -f 2)
    FILEDAY=$(echo "${FILEYEARPATH%%/*}" | cut -d - -f 3)
    FILETIME=$(echo "${FILEYEARPATH%%/*}" | cut -d - -f 4)
...
deleteBackups() {
    ...
    for f in $(find ${BACKUPDIR} -name "*.enc" -type f); do
        KEEPFILE="NO"
        getFileDate "$f"
        ...
        if [ $? == 0 ]; then
            ...
            if [ ${KEEPFILE} == "NO" ]; then
                ...
                #Check for empty directories and delete
                for d in $(find ${BACKUPDIR} -type d -empty); do
                    rm -rf "${d}"
                    log "DELETED empty directory ${d}"
                done
...
runRemotely() {
    ...
    REMOTEHOSTNAME="/tmp/hostname"
	echo "BACKUPHOSTNAME=$(hostname)" > ${REMOTEHOSTNAME}
